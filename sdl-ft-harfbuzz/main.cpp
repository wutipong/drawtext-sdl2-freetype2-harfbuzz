...
void CalculateSurfaceBound(hb_glyph_info_t *glyph_infos,
                           ...
                           const unsigned int &glyph_count, const FT_Face &face,
                           SDL_Rect &rect,
...
void CreateTexture(const std::wstring &text, const SDL_Color &color,
                   const Font &font, SDL_Renderer *&renderer,
                   SDL_Texture *&target, SDL_Rect &rect,
                   const FT_Int32 &flags = FT_LOAD_DEFAULT) {
  ...
  CalculateSurfaceBound(glyph_infos, glyph_positions, glyph_count, font.face,
                        rect, flags);
  ...
  for (unsigned int i = 0; i < glyph_count; i++) {
    FT_Load_Glyph(font.face, glyph_infos[i].codepoint, FT_LOAD_RENDER | flags);

    SDL_Texture *glyph_texture =
        CreateTextureFromFT_Bitmap(renderer, font.face->glyph->bitmap, color);

    SDL_Rect dest;
    SDL_QueryTexture(glyph_texture, NULL, NULL, &dest.w, &dest.h);
    dest.x = x + (font.face->glyph->metrics.horiBearingX >> 6) +
             (glyph_positions[i].x_offset >> 6);
    dest.y = baseline - (font.face->glyph->metrics.horiBearingY >> 6) -
             (glyph_positions[i].y_offset >> 6);
...
int main(int argc, char **argv) {
  ...
  SDL_Window *window =
      SDL_CreateWindow("SDL2 - FreeType 2 - HarfBuzz", SDL_WINDOWPOS_UNDEFINED,
  ...
  while (true) {
    ...
    CreateTexture(text, color, font, renderer, texture, rect,
                  FT_LOAD_NO_HINTING);
